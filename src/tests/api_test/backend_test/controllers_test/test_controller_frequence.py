...
import src.api.backend.controllers.controller_frequence as controller_frequence


@pytest.fixture
def client():
    app = FastAPI()
    app.include_router(controller_frequence.router)
...
def test_build_frequence_model_success(monkeypatch):
...
def test_build_frequence_model_missing_file(monkeypatch):
    ...
    with pytest.raises(controller_frequence.HTTPException) as exc_info:
        controller_frequence._build_frequence_model()

    assert exc_info.value.status_code == 500
    assert "Model JSON introuvable" in exc_info.value.detail


def test_build_frequence_model_invalid_artifact(monkeypatch):
    mock_model = MagicMock()
...
def test_health_predictio_frequence_ok(client, monkeypatch):
    monkeypatch.setattr(controller_frequence, "FREQUENCE_MODEL", MagicMock())
    monkeypatch.setattr(controller_frequence, "FREQUENCE_MODEL_LOAD_ERROR", None)
    monkeypatch.setattr(controller_frequence.os.path, "exists", lambda _: True)
...
def test_prediction_success(client, valid_payload, monkeypatch):
    ...
    monkeypatch.setattr(controller_frequence, "FREQUENCE_MODEL", mock_model)
    monkeypatch.setattr(controller_frequence, "FREQUENCE_MODEL_LOAD_ERROR", None)
...
def test_prediction_model_unavailable(client, valid_payload, monkeypatch):
    ...
    assert "Le modèle de fréquence n'est pas disponible" in response.json()["detail"]
