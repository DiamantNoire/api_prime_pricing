...
# Ce script orchestre l'entraînement du modèle de fréquence,
...
if __name__ == "__main__":
    ...
    # --- Output ---
    OUTPUT_FEATURE_ENGINEERING_FREQUENCE_PATH = os.path.join(
        DATA_DIR, "output", "feature_engineering", "features_frequence.pickle"
    )
    OUTPUT_MODEL_FREQUENCE_PATH = os.path.join(
        DATA_DIR, "output", "modeles", "model_frequence.pickle"
    )
    OUTPUT_METRICS_FREQUENCE_PATH = os.path.join(
        DATA_DIR, "output", "metrics", "metrics_frequence.json"
    )
    OUTPUT_TEST_PRED_FREQUENCE_PATH = os.path.join(
        DATA_DIR, "output", "predictions", "test_predictions_frequence.csv"
    )
    OUTPUT_COMPLETE_ARTIFACT_FREQUENCE_PATH = os.path.join(
        PROJECT_ROOT, "output_models", "modeles", "model_frequence.json"
    )
    OUTPUT_SYNTHETIC_ARTIFACT_FREQUENCE_PATH = os.path.join(
        DATA_DIR, "output", "artifacts", "synthetic_artifact_frequence.json"
    )

    os.makedirs(
        os.path.dirname(OUTPUT_FEATURE_ENGINEERING_FREQUENCE_PATH), exist_ok=True
    )
    ...
    os.makedirs(
        os.path.dirname(OUTPUT_SYNTHETIC_ARTIFACT_FREQUENCE_PATH), exist_ok=True
    )
    ...
    y_pred_train_freq = run_step(
        "Predict train frequence", model_frequence.predict, X_train_copie_freq
    )

    y_pred_valid_freq = run_step(
        "Predict valid frequence", model_frequence.predict, X_valid_copie_freq
    )

    y_pred_test_freq = run_step(
        "Predict test frequence", model_frequence.predict, df_test_freq
    )
    metrics = run_step(
    ...
    _generer_csv_pred_freqence(
        df=df_test, y_pred=y_pred_test_freq, path=OUTPUT_TEST_PRED_FREQUENCE_PATH
    )
