...
def _ajouter_contenu(nom_de_page:str):
    """Injecte le contenu dans les conteneurs posés par la disposition (cache-only)."""
    layout = (st.session_state.get("diposition"))
    if not layout:
        # Recrée la disposition si nécessaire
        try:
...
try:
    ...
    _ajouter_contenu(nom_de_page=titre_page)
