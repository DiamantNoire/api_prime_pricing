...
OUTPUT_ARTIFACT_PATH = os.path.join(PARENT_DIR, 'sorties', 'amelioration', 'pipeline_amount_no_outliers.pickle')
...
if not df_outliers.empty:
    df_train_no_outliers = df_train[~df_train.index.isin(df_outliers.index)]
else:
...
df_train_no_outliers = add_interaction(df_train_no_outliers.copy())
...
X_train, X_valid, y_train, y_valid = run_step('Train/Validation split',
                                              train_test_split,
                                              df_train_no_outliers.drop(columns=['montant_sinistre']),
                                              df_train_no_outliers['montant_sinistre'],
                                              test_size=0.2,
                                              random_state=42,
                                              shuffle=True)
...
metadata_amount = {
    ...
    "feature_engineering_result": {
        "objectif": "amelioration_v1_feature_engineering_no_outliers",
        ...
        "resume_traitement_feature": (
            "Objectif du run : ajout d'une interaction simple (age_conducteur1 x bonus) en plus du preprocessing, sans outliers."
        )
    },
    "model_selection_result": {
        "objectif": "amelioration_v1_selection_modeles_no_outliers",
        "results": results,
        ...
        "resume_traitement_modeles": (
            "Objectif du run : comparaison multi-modèles sur le même pipeline de features, sans outliers."
        )
    }
}
run_step('Sauvegarde du pipeline Amount (no outliers)', model_amount.save_model, model_amount.models_[best_model], OUTPUT_ARTIFACT_PATH, metadata=metadata_amount)
