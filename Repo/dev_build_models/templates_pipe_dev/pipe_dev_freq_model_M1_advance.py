...
if __name__ == "__main__":
    ...
    TRAIN_PATH = os.path.join(DATA_DIR, 'path_train.csv')
    TEST_PATH = os.path.join(DATA_DIR, 'path_test.csv')
    ...
    # =======================================================
    OUTPUT_ARTIFACT_PATH = os.path.join(DATA_DIR, 'path/name_of_model.pickle')
    ...
    fe_freq = Feature_Engineer_Amount(amount_process=Amount_Preprocessing())
    run_step('Construction du feature engineer Amount',
             #fe_freq.build_feature_engineer,
             # arguments de la fonction de construction de la feature engineering
            # arg1=val
            # etc...
            )
    ...
    contribution_drift_df = contribution_drift_df.sort_values(
                                                            # 'drift_abs', 
                                                            # ascending=False
                            )
    overfit_rmse_gap = metrics_amount_train.get(
                                                # "RMSE", 
                                                # 0.0
                                                ) - metrics_amount_valid.get(
                                                                            # "RMSE", 
                                                                            # 0.0
                                                                            )
    overfit_rmse_ratio = (
    ...
    #         try:
    #             imp = permutation_importance(model_freq, X_train_transfomed[[f]], y_train, n_repeats=3, random_state=42, scoring='neg_mean_squared_error')
    ...
    # --- Build metadata FREQ Model M1  ---
    metadata_amount = {
        # "target": "montant_sinistre",
        # "objectif": "baseline",
        # "feature_engineering_steps": {
        #     "booking_appleid": fe_freq.booking_applied,
        #     "preprocessing_map": fe_freq.preprocessing_map
        # },
