...
class Model_Prediction_Amount(BaseEstimator):
    ...
    def __init__(self,
        ...
        # Conteneur des artefacts produits lors du fit : {model_name: artifact_dict}
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, model_name: str = None, kfold=None):
        ...
        if model_name is not None:
            ...
            self.history_[model_name] = [(self.selected_features_.copy(), None)]
        ...
        # Sinon on fait un entrainement sur tous les modèles et on retourne le dict de dictionnaire entrainés
        else:
            for name, model in self.models_.items():
                ...
                # Mise à du dictionnaire de modèles entrainés
                model_improved = model # Changement de nom pour garder une trace visible du travail d'entrainement
                self.models_[name] = model_improved
                self.selected_features_ = self.models_features_[model_improved].copy()
                self.selected_features_keep_ = self.selected_features_.copy()
                self.selected_features_investigate_ = self.models_contribution_[model_improved].loc[~keep_mask, 'feature'].tolist()
                self.contribution_drift_df_ = self.models_contribution_[model_improved].sort_values('drift_abs', ascending=False)
                self.history_ = self.models_history_[model_improved].copy()
                self.model_name_ = model_improved
                self.model_ = self.models_[model_improved]
        return self
    ...
    def save_model(self, model_name_or_model, filepath: str, metadata: Optional[Dict[str, Any]] = None):
        """Sauvegarde l'artefact structuré pour un modèle.

        - Si `model_name_or_model` est une chaîne correspondant à une clé
          dans `self.models_artifacts_`, on sauvegarde cet artefact tel quel.
        - Si c'est un objet modèle, on reconstruit un artefact à partir de
          l'état courant (`self.selected_features_`, etc.).
        """
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        artifact = None

        # cas: on a reçu un nom de modèle
        if isinstance(model_name_or_model, str):
            name = model_name_or_model
            artifact = deepcopy(self.models_artifacts_.get(name)) if name in self.models_artifacts_ else None
            model_obj = self.models_.get(name)
        else:
            model_obj = model_name_or_model

        # si aucun artefact trouvé, construire à partir de l'état courant
        if artifact is None:
            # contribution_drift_df_ peut être DataFrame ou liste; conserver DataFrame avant sérialisation
            contrib_df = self.contribution_drift_df_.copy() if isinstance(self.contribution_drift_df_, pd.DataFrame) else pd.DataFrame(self.contribution_drift_df_)
            artifact = {
                'model': model_obj,
                'selected_features': list(self.selected_features_),
                'selected_features_keep': list(self.selected_features_keep_),
                'selected_features_investigate': list(self.selected_features_investigate_),
                'numeric_features': list(self.numeric_features_),
                'fill_values': dict(self.fill_values_),
                'contribution_drift_df': contrib_df,
                'history': deepcopy(self.history_),
                'config': {
                    'cv': self.cv,
                    'scoring': self.scoring,
                    'max_features': self.max_features,
        ...
        # Avant sérialisation : convertir DataFrame en structure sérialisable
        def _serialize_art(a: Dict[str, Any]) -> Dict[str, Any]:
            s = deepcopy(a)
            if 'contribution_drift_df' in s and isinstance(s['contribution_drift_df'], pd.DataFrame):
                s['contribution_drift_df'] = s['contribution_drift_df'].to_dict(orient='records')
            return s

        # Construire le contenu global à sauvegarder :
        # - si on a des artefacts dans self.models_artifacts_, inclure la collection complète
        # - inclure aussi l'artefact courant (serializable)
        container: Dict[str, Any] = {}
        if isinstance(self.models_artifacts_, dict) and len(self.models_artifacts_) > 0:
            # sérialiser chaque artefact stocké
            all_arts = {}
            for k, v in self.models_artifacts_.items():
                all_arts[k] = _serialize_art(v)
            container['models_artifacts'] = all_arts

        container['artifact'] = _serialize_art(artifact)
        # marquer éventuellement le nom du modèle dans les métadonnées
        if isinstance(model_name_or_model, str):
            container['artifact'].setdefault('metadata', {})
            container['artifact']['metadata'].setdefault('model_name', model_name_or_model)

        with open(filepath, 'wb') as f:
            pickle.dump(container, f)

    def load_model(self, filepath: str):
        """Charge un artefact (format aligné avec `models_artifacts_`) et met à jour l'état.

        Si l'artefact contient une clef `'model'` et d'autres clés attendues,
        on recharge les champs correspondants (`model_`, `selected_features_`, ...)
        et on retourne le dict chargé.
        """
        try:
            with open(filepath, 'rb') as f:
                loaded = pickle.load(f)
            ...
            if isinstance(loaded, dict):
                # Si le fichier contient une collection d'artefacts
                if 'models_artifacts' in loaded and isinstance(loaded['models_artifacts'], dict):
                    self.models_artifacts_ = {}
                    for name, art in loaded['models_artifacts'].items():
                        art_copy = deepcopy(art)
                        if 'contribution_drift_df' in art_copy and isinstance(art_copy['contribution_drift_df'], list):
                            art_copy['contribution_drift_df'] = pd.DataFrame(art_copy['contribution_drift_df'])
                        self.models_artifacts_[name] = art_copy

                # si le container a un 'artifact' unique, l'appliquer comme état courant
                candidate = None
                if 'artifact' in loaded:
                    candidate = loaded['artifact']

                # format ancien compatibilité : clefs terminant par '_' (ex: 'model_')
                if candidate is None and 'model_' in loaded:
                    candidate = loaded

                if candidate is not None:
                    # ancien format
                    if 'model_' in candidate:
                        self.model_ = candidate.get('model_', self.model_)
                        self.selected_features_ = candidate.get('selected_features_', self.selected_features_)
                        self.selected_features_keep_ = candidate.get('selected_features_keep_', self.selected_features_keep_)
                        self.selected_features_investigate_ = candidate.get('selected_features_investigate_', self.selected_features_investigate_)
                        self.numeric_features_ = candidate.get('numeric_features_', self.numeric_features_)
                        self.fill_values_ = candidate.get('fill_values_', self.fill_values_)
                        contribution_drift_records = candidate.get('contribution_drift_df_', [])
                        self.contribution_drift_df_ = pd.DataFrame(contribution_drift_records)
                        self.history_ = candidate.get('history_', self.history_)
                        config = candidate.get('config', {})
                    else:
                        # nouveau format artefact
                        self.model_ = candidate.get('model', self.model_)
                        self.selected_features_ = candidate.get('selected_features', self.selected_features_)
                        self.selected_features_keep_ = candidate.get('selected_features_keep', self.selected_features_keep_)
                        self.selected_features_investigate_ = candidate.get('selected_features_investigate', self.selected_features_investigate_)
                        self.numeric_features_ = candidate.get('numeric_features', self.numeric_features_)
                        self.fill_values_ = candidate.get('fill_values', self.fill_values_)
                        contrib = candidate.get('contribution_drift_df', None)
                        self.contribution_drift_df_ = pd.DataFrame(contrib) if contrib is not None else self.contribution_drift_df_
                        self.history_ = candidate.get('history', self.history_)
                        config = candidate.get('config', {})

                    # appliquer config commun
                    self.cv = config.get('cv', self.cv)
                    self.scoring = config.get('scoring', self.scoring)
                    self.max_features = config.get('max_features', self.max_features)
                    self.random_state = config.get('random_state', self.random_state)
                    ...
                    # si l'artefact a metadata.model_name, enregistrer
                    meta = candidate.get('metadata', {}) or {}
                    model_name = meta.get('model_name')
                    if model_name:
                        # stocker l'artefact brut (non-serialisé) dans models_artifacts_
                        art_to_store = deepcopy(candidate)
                        if 'contribution_drift_df' in art_to_store and isinstance(art_to_store['contribution_drift_df'], list):
                            art_to_store['contribution_drift_df'] = pd.DataFrame(art_to_store['contribution_drift_df'])
                        self.models_artifacts_[model_name] = art_to_store

                    return loaded

            # fallback: retourner l'objet brut
            self.model_ = loaded
            return loaded
        except Exception as e:
            print(f"Erreur lors du chargement du modèle: {e}")
...
class Analyses_Comparaisons_Affichages_Resultats:
    ...
    def affichage_artefact(self, path:str)-> None:
        ...
        Args:
            path (str): le chemin de sauvegarde du modèle
        """
