...
#--*- coding: utf-8 -*-
# Script Amount Catégoriel (features catégorielles, booking conforme)
# =============================================
...
if __name__ == "__main__":
    ...
    # =======================================================
    OUTPUT_ARTIFACT_PATH = os.path.join(DATA_DIR, 'sorties/pipeline/pipeline_amount_categorical.pickle')
    ...
    # Booking métier pour features catégorielles
    categorical_features = [
        'marque_vehicule', 'type_vehicule', 'usage_vehicule', 'genre_vehicule',
        'type_contrat', 'type_garantie', 'type_client', 'region', 'code_postal',
        'civilite_conducteur1', 'civilite_conducteur2', 'situation_familiale',
        'type_permis', 'type_proprietaire', 'type_assureur', 'type_sinistre'
    ]
    ...
    run_step('fit du feature engineer Amount sut train',
             fe_amount.fit,
             X=X_train,
             y=y_train)
    ...
    metadata_amount = {
        ...
        "resume_traitement_feature": (
            "Objectif du run : catégoriel (encodage avancé des variables catégorielles, imputation, normalisation, puis sélection automatique des variables par permutation_importance).\n"
            f"Features à investiguer non sélectionnées : {selected_features_investigate if 'selected_features_investigate' in locals() else []}.\n"
            "Voir importance/contribution dans les autres champs de métadonnées."
        )
    }
    run_step('Sauvegarde du pipeline Amount (catégoriel)',
             model_amount.save_model,
             model_amount.model_name_,
             OUTPUT_ARTIFACT_PATH,
             metadata=metadata_amount)
    print("\n[METADATA PIPELINE AMOUNT CATEGORIAL]")
